...
const ResumeTemplate = ({template}) => {
    ...
    return (
     ...
     <div className="border-4 border-cyan-500">
          ...
          <p className="text-gray-600 mt-2">{email} | {phone} | {address}</p>
        </header>
  
        {/* Career Objective */}
        {careerObjective && (
          <section className="mb-8">
            <h2 className="text-xl font-semibold text-gray-700 border-b pb-2">Career Objective</h2>
        ...
        {certifications && certifications.length > 0 && (
          <section className="mb-8">
            <h2 className="text-xl font-semibold text-gray-700 border-b pb-2">Certifications</h2>
            <ul className="mt-4 space-y-2">
              {certifications.map((cert, index) => (
                <li key={index}>
                  <h3 className="text-lg font-semibold">{cert.title} - {cert.institution}</h3>
          ...
          <section className="mb-8">
            ...
            <ul className="mt-4 space-y-4">
              {workExperience.map((exp, index) => (
                <li key={index}>
                  <h3 className="text-lg font-semibold">{exp.jobTitle} - {exp.company}</h3>
        ...
        {extraCurricularActivities && extraCurricularActivities.length > 0 && (
          <section className="mb-8">
            <h2 className="text-xl font-semibold text-gray-700 border-b pb-2">Extra Curricular Activities</h2>
            <ul className="mt-4 space-y-2">
